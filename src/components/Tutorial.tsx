...
export function Tutorial({ onComplete }: { onComplete: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#050505]/90 backdrop-blur-xl animate-in fade-in duration-500 p-6">
            <div className="relative w-full max-w-2xl bg-gradient-to-b from-gray-900/90 to-black/90 rounded-[2rem] border border-white/10 shadow-2xl p-8 md:p-12 overflow-hidden flex flex-col min-h-[500px]">
                ...
                {/* Progress Indicators */}
                <div className="flex gap-2 justify-center mb-12 relative z-10">
                    {steps.map((_, idx) => (
                        <div
                            key={idx}
                            className={cn(
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center relative z-10 slide-in-from-right-8 animate-in duration-500">
                    <div className="mb-8 p-6 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-sm shadow-xl inline-flex animate-bounce-subtle">
                ...
                <div className="mt-12 flex items-center justify-between relative z-10">
                    ...
                    <button
                        onClick={handleNext}
                        className={cn(
                            "flex items-center gap-2 px-8 py-4 rounded-xl font-bold transition-all duration-300 shadow-lg active:scale-95",
